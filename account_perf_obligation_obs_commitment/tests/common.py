...
class PerfObligationObsCommitmentCommon(TransactionCase):
    ...
    def _create_and_post_move(self, journal, line_specs, date="2026-01-01"):
        """Create and post a move linking lines to an obligation through the
        base `perf_obligation_id` field (recognition journal entries)."""
    ...
    def _create_and_post_obs_move(
        self, commitment_account, debit, credit, po, date="2026-01-01"
    ):
        """Create and post a manual entry directly in the OBS journal,
        simulating an out-of-band movement of the commitment account.
        """
        move = self.env["account.move"].create(
            {
                "journal_id": self.obs_journal.id,
                "date": date,
                "line_ids": [
                    Command.create(
                        {
                    ...
                    Command.create(
                        {
                            "account_id": self.obs_counter_account.id,
                            "debit": credit,
                            "credit": debit,
                            "name": "Test",
                        }
    ...
    def _get_obs_lines(self, po):
        """Return all OBS commitment move lines linked to *po*."""
