...
class PerfObligationObsCommon(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        ...
        cls.obs_expense_account = Account.create(
            {
                "name": "OBS Expense Commitment",
                "code": "8EOBST",
                "account_type": "off_balance",
            }
        )
        cls.obs_counterpart_account = Account.create(
            {
                "name": "OBS Counterpart",
                "code": "9COBST",
                "account_type": "off_balance",
            }
        )
        ...
        cls.company.write(
            {
                ...
                # off-balance sheet
                "po_obs_journal_id": cls.obs_journal.id,
                "po_obs_income_account_id": cls.obs_income_account.id,
                "po_obs_expense_account_id": cls.obs_expense_account.id,
                "po_obs_counterpart_account_id": cls.obs_counterpart_account.id,
            }
        )
    ...
    def _get_obs_lines(self, po):
        """Return all move lines in the OBS journal linked to *po*."""
